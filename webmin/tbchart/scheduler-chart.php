...
if($_GET['printable']==1){
	...
	echo '<link rel="stylesheet" media="all" type="text/css" href="../css/jquery-ui.css" />';
?>
<base href="http://<?php echo $_SERVER['HTTP_HOST']; ?>/webmin/" target="_blank">
<style>
	body, td, th, div{ 
	...
	td, th{ border-top:thin solid #333; border-right:thin solid #333; padding:4px; }

.hour_th{ font-weight: bold !important; color:#fff !important; background-color:#333 !important; }
.min_th{ font-size:10px !important; }
...
if($_SESSION['login']['type']>2){
	die('<br><br>&nbsp;&nbsp;&nbsp;&nbsp;You do not have enough privilege to view this page.');
}
...
$sql = "SELECT description FROM `type` WHERE staff=1";
$qry = mysql_query($sql);
$cnt=0;
while($res = mysql_fetch_assoc($qry)){
	echo '.'.str_replace(' ','',$res['description']).'{ background-color: '.$colors[$cnt].' }';
	$cnt++;
}
?>
...
<script>
			...
			$(document).ready(function(){
				...
				$('td.timeslot').click(function(e){
					...
					jQuery.ajax({
							 url: "tablechart/add_schedule.php",
							 type: 'POST',
			...
			$('.table_block').click(function(e){
					...
					jQuery.ajax({
							 url: "tablechart/update_staff_schedule.php",
							 type: 'POST',
							 data: 'parameters='+parameters,
							 success: function(value){
...
<div class="container avails-grid" id="table_chart">
    ...
    <?php if($_GET['printable']==0){ ?><div style="float:right;"><a href="tablechart/scheduler-chart.php?printable=1" target="_blank">View Printable</a></div><?php } ?>
            ...
            <tbody>
				...
				$staff_sql = "SELECT a.id, CONCAT(fname,' ', lname) AS staff_name, t.description  
							  ...
							  WHERE a.deleted=0 AND  a.type_id=t.id 
							  	AND type_id IN (SELECT id FROM `type` WHERE staff=1)  
							  ORDER BY t.description ASC, fname, lname ASC";
				$staff_qry = mysql_query($staff_sql);
				while($staff_res = mysql_fetch_assoc($staff_qry)){
				?>
                <tr>
                	<td valign="middle" nowrap="nowrap">
                      <div>	
						<div style="float:left; margin-top:2px;"><?php echo $staff_res['staff_name']; ?></div>
                      </div>
                    </td>
                	<td valign="middle" nowrap="nowrap">
                      <div>	
						<div class="user_title <?php echo str_replace(' ','',$staff_res['description']); ?>"><?php echo $staff_res['description']; ?></div>
                      </div>
                    </td>
				...
				while($time<=$end){
					...
					if($block_ends>$time){
						$time_slot_class = 'reserved '.str_replace(' ','',$staff_res['description']);	
						$open_slot = 0;
					...
					$table_reserve_sql = "SELECT 'sched' AS sched_type, id, end_time, start_time, ROUND(TIME_TO_SEC((TIMEDIFF(end_time, start_time)))/60) AS duration, (TIMEDIFF(end_time, start_time)) AS diff_hr_min  
										  ...
										  SELECT 'shift' AS sched_type, s.id, s.end_time, s.start_time, ROUND(TIME_TO_SEC((TIMEDIFF(s.end_time, s.start_time)))/60) AS duration, (TIMEDIFF(s.end_time, s.start_time)) AS diff_hr_min  
											...
											WHERE sr.account_id_1=".$staff_res['id']." AND sr.sched_02=s.id AND sr.option=1 AND sr.deleted=0 AND sr.approve=1 
												  ...
												  AND
												  ( 
													STR_TO_DATE('".date('m/d/Y',strtotime($date_selected))."', '%m/%d/%Y') BETWEEN STR_TO_DATE(s.valid_from, '%m/%d/%Y') AND STR_TO_DATE(s.valid_until, '%m/%d/%Y') 
												  ) 
												  AND s.deleted=0
												  AND s.days LIKE '%".date('D',strtotime($date_selected))."%' AND s.start_time='".date('H:i',$time)."' 												  					
...
<?php
if($_GET['printable']==1){ echo 'window.print();'; }
else{ 
?>
