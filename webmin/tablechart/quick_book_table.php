...
jQuery( "#time_start" ).change(function() {
  // Check input( $( this ).val() ) for validity here
	...
	jQuery.ajax({
			 url: "tablechart/timeDiffMins.php",
			 type: 'POST',
			 data: 'start='+start_time+'&end='+end_time+'&date_selected='+date_selected,
			 success: function(value){
				jQuery('#duration').html(value);
			 }
	});		
...
jQuery( "#save_booking" ).click(function() {
	...
	if(customer_id>0){
		...
		if(extra_chairs>0){
			confirm('Warning: You have set '+extra_chairs+' extra chair(s).');
		}
...
jQuery( "#save_walkin" ).click(function() {
	...
	if(extra_chairs>0){
		extra_desc = 'Warning: You have set '+extra_chairs+' extra chair(s).\n\n';
	}

	  
	if(confirm(extra_desc+'Do you really want to book this as WALK-IN?')){		 
...
jQuery('#signatory').typeahead({
     name: 'signatory',
	 valueKey: 'name',
     local: [<?php echo $account_list; ?>]		
	}).on('typeahead:selected', function($e, datum){
        jQuery('#signatory').attr('data-rel',datum["id"]);
	})
...
jQuery('#customer_search').keyup(function(){
	var val = jQuery(this).val();
	
	if(isNaN(val)){
		jQuery('#first_name').val(val);
	}
	else{
		jQuery('#phone_number').val(val);
	}
...
jQuery(function(){
	...
	var array = [<?php echo $days_disabled; ?>];

	jQuery( "#date_selected" ).datepicker(
	
	{
		minDate: 0,
		onSelect: function(selectedDate){
			jQuery('#table_list').html('Klicka på “Uppdatera bordslista” för tillgängliga bord.');	
			jQuery('#selected_tables').html('0');
			jQuery('#selected_seats').html('0');
			jQuery('#seats_difference').html('');
		},
...
<table width="100%" cellspacing="2" cellpadding="2">
  <tr>
    ...
    <td colspan="4" valign="middle">
    	<?php //<strong><?php echo dayName(date('D, d M Y',strtotime($parameters[0]))); </strong> ?>
        <input type="text" name="date_selected" id="date_selected" value="<?php echo $parameters[0]; ?>" style="width:100px;" />
    </td>
    <td valign="middle">&nbsp;</td>
	...
	<tr>
	  ...
	  <td align="center" valign="middle">:&nbsp;</td>
	  <td valign="middle">
	  <input type="text" name="pax" id="pax" style="width:40px;" />
	  </td>
	  <td valign="middle"><div align="right" style="margin-left:8px; vertical-align:top;">Extra Chairs</div></td>
	  <td valign="right">:</td>
	  <td valign="middle"><input type="text" name="extra_chairs" id="extra_chairs" style="width:40px;" /></td>
    	<td valign="middle">&nbsp;</td>
	...
	<tr>
	  ...
	  <td align="center">:</td>
	  <td colspan="4"><div id="selected_seats">0</div></td>
	  <td>&nbsp;</td>
  ...
  <tr>
    <td align="right" valign="middle">Signatory</td>
    <td align="right" valign="middle">:&nbsp;</td>
    <td colspan="8"><input type="text" id="signatory" style="width:250px !important;" /></td>
  </tr>
	<tr>
	  <td colspan="10" align="center">
      <input type="button" id="save_walkin" value="Save As Wak-In" style="padding:4px;" /></td>
    </tr>
  <tr>
  ...
  <tr>
    <td align="right" valign="middle">Gäst </td>
    <td align="right" valign="middle">:&nbsp;</td>
    <td colspan="8">
  ...
  <tr class="new_customer">
    <td align="center" valign="top" colspan="10">Ingen gäst med dessa uppgifter hittades. Lägg till gäst nedan.</td>
  </tr>  
  <tr class="new_customer">
