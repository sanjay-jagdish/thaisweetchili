    	...
    	<table cellpadding="0" cellspacing="0" border="0" class="table table-condensed table-chart table-bordered" style="clear:none !important;">
            ...
            <tbody>
				...
				while($time<=$end){
					...
					$table_reserve_sql = "SELECT *, ROUND(TIME_TO_SEC((TIMEDIFF(end_time, start_time)))/60) AS duration, (TIMEDIFF(end_time, start_time)) AS diff_hr_min  
										  FROM schedule
					...
					if($table_reserve_num==1){
						...
						//check if a shift_request has been approved
						$sr_sql = "SELECT * FROM shift_request WHERE deleted=0 AND approve=1 AND (sched_01=".$table_reserve['id']." OR sched_02=".$table_reserve['id'].")";
						$sr_qry = mysql_query($sr_sql);
						$sr_num = mysql_num_rows($sr_qry);
						
						$sr_note = '';
						if($sr_num>0){
							$sr_res = mysql_fetch_assoc($sr_qry);
	
							if($table_reserve['id']==$sr_res['sched_01']){ $shift_sched = $sr_res['sched_02']; }else{ $shift_sched = $sr_res['sched_01']; }
							//$sr_note = '***SR['.$shift_sched.']***';
						}
						...
						$table_block = '<div class="table_block" style="width:'.$time_blocks_width.'px;" data-rel="'.$table_reserve['id'].'">'.$sr_note.'
