...
jQuery( "#res_status" ).change(function() {
  // Check input( $( this ).val() ) for validity here
	var booking_id = <?php echo $res_id; ?>;
	var res_status = jQuery( "#res_status" ).val();
	var date_selected = jQuery( "#date_selected" ).val();

	jQuery.ajax({
			 url: "tablechart/booking_status_update.php",
			 type: 'POST',
			 async: false, 
			 data: 'res_id='+booking_id+'&status='+res_status+'&date_selected='+date_selected,
			 success: function(value){
				 value_res = value.split('|'); //status key | color class 
				 jQuery('#res_status_circ').removeAttr( "style" );
				 jQuery('#res_status_circ').removeClass();
				 jQuery('#res_status_circ').addClass('reserve_status');
				 jQuery('#res_status_circ').addClass(value_res[1]);
				 jQuery('#res_status_circ').html(value_res[0]);
					
					jQuery.ajax({
						
							url: "tablechart/calendar-data.php",
							type: 'GET',
							data: 'date='+value_res[2],
							success: function(value_){
								jQuery('.fadediv, .loaddiv').fadeOut();
								jQuery('#calendar-wrapper').html(value_);
								// alert(value);
							}
						});
...
jQuery( "#save_booking" ).click(function() {
  	...
  	//if(customer_id>0){
		...
		if(seats_sel>pax){
			...
			jQuery.ajax({
					 ...
					 type: 'POST',
					 async: true,
					 data: 'res_id='+res_det_id+'&date_selected='+date_selected+'&time_start='+time_start+'&time_end='+time_end+'&table_id='+table_id+'&pax='+pax+'&notes='+notes+'&res_det_id='+res_det_id+'&customer_id='+customer_id+'&extra_chairs='+extra_chairs,
					 success: function(value){
  ...
  //} //if client or customer id is present
...
$(document).ready(function(){
	...
	$('#cancel_booking').click(function(e){
		...
		jQuery.ajax({
				 ...
				 type: 'POST',
				 data: 'parameters='+parameters+'&date='+date_selected,
				 success: function(value){
					jQuery('.fadediv, .loaddiv').fadeOut();
...
$res_details_sql = "SELECT r.account_id AS id, r.date, r.time, 
					ADDTIME(TIME(r.time), SEC_TO_TIME(duration*60)) AS end,
					 r.duration, a.fname, a.lname, a.phone_number, 
						r.number_people AS pax, extra_chairs, r.note, r.status 
						FROM reservation r 
...
<table width="100%" cellspacing="2" cellpadding="2">
  ...
  <tr>
	...
	<tr>
	  ...
	  <td align="center" valign="middle">:&nbsp;</td>
	  <td valign="middle">
	  <input type="text" name="pax" id="pax" style="width:25px;" value="<?php echo $res_details_res['pax']; ?>" />
	  <div style="float:right;">
      Extra Chairs: 
        <input type="text" name="extra_chairs" id="extra_chairs" style="width:25px;" value="<?php echo $res_details_res['extra_chairs']+0; ?>" />
      </div>
    	</td>
    	<td valign="middle">&nbsp;</td>
