<?php
	...
	$sql_counts = mysql_query("SELECT id FROM ".$tablename." where deleted = 0"); //echo "SELECT id FROM ".$tablename." where deleted = 0 ";  //victor
	...
	if($endDate == $today){
		...
		if($thour==$endHour && $ttempmins > $endMinutes){
			echo 1; //victor
		}else{
			echo 0;	
		}
