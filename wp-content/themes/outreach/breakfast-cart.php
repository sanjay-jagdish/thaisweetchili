	...
	<div class="forcatering">
        <div class="cartbox">
        	<table align="center" class="rwd-table">
                ...
                <?php
                	$q=mysql_query("select m.name as menu, rd.notes as notes, rd.price as price, rd.quantity as quantity, rd.id as id, m.image as img, m.id as menu_id, rd.lunchmeny as lunchmeny, m.single_option as single_option from reservation_detail as rd, breakfast_menu as m where m.id=rd.menu_id and m.deleted=0 and rd.reservation_id='".$reservation_id." and rd.lunchmeny=2'") or die(mysql_error());
				...
				?>
                	<tr class="<?php echo $theclass.' '.$pimg; ?> menudetails menudetail-<?php echo $r['id'];?>">
                    	...
                    	<td data-th="Rätt:" valign="top"><?php echo $r['menu']; ?>
                        	 
                            <?php $opt_q = mysql_query("select * from breakfast_menu_options where menu_id = $r[menu_id]") or die(mysql_error());
                             ...
                             <?php if($r['quantity']>1){
								echo '<div style="padding-left:10px;" class="cart-opt" id="opt-holder-'.$r['id'].'">';
								 for ($x=1; $x <= number_format($r['quantity']); $x++) {
										...
										$query_opt = mysql_query("select * from breakfast_menu_options where menu_id = $r[menu_id]") or die(mysql_error());
...
<script type="text/javascript">
	...
	$(function(){
		...
		if(checker<=0){
			$('.fade, .takeawaycart-detail, .breakfastcart').fadeOut();
		...
		$('.remove-tillval').click(function(){
			...
			if(quan<1){
				...
				$.ajax({
						url: "<?php echo $theurl; ?>/breakfast-cart.php",
						type: 'POST',
						async: false,
		...
		$('.open-takeaway-cart').click(function(){
			...
			var total = $(this).attr('data-rel');
			  $.ajax({
				...
				type: 'POST',
				async: false,
				data: 'total='+encodeURIComponent(total)+'&tablename='+encodeURIComponent('breakfast_settings'),
				success: function(value){
