...
function goScroll(id){
  ...
  $(function(){
		...
		$(document).ready(function() {
			...
			calendarDate = $.datepicker.formatDate('yy-mm-dd', myDate);
			...
			if((compareTimes(curTime,time1) == 1 || compareTimes(curTime,time1) == 0) && (compareTimes(curTime,time2) == -1 || compareTimes(curTime,time2) == 0) && ( +curDay == +calDay) && Number(<?php echo $thour;?>)>0){
				...
				if(isMoveDate == 1){
					//console.log('will move '+$('#currenDate').val());
					 ...
					 $("#datepickerinline").datepicker( "option", "minDate", new Date(yy+'-'+mm+'-'+dd) );
					 $('#datepickerinline').datepicker('setDate', new Date(yy+'-'+mm+'-'+dd));
					 $('#datepicker2').val('Bearbetar...');
					 
					 calendarDate = new Date(yy+'-'+mm+'-'+dd);
					 
					 console.log('Condition 1 '+calendarDate);
					 
					 setTimeout(function(){
						$('.ui-timepicker-div dl').html('<div class="beforetime">Välj datum</div>');
						$('.ui-datepicker-close ').hide();
						$('.steps-button-2 input').prop('disabled',true);
						$('.steps-process .stp-nav .btn.next').hide();
					},300);
				}
				...
				if(+curDay == +calDay && (compareTimes(curTime,time2)==1)){
					 ...
					 calendarDate = $.datepicker.formatDate('yy-mm-dd', new Date(yy+'-'+mm+'-'+dd));
					 console.log('Condition 3 '+calendarDate);
					...
					//console.log('will move '+$('#currenDate').val());
					 ...
					 console.log('Condition 4 '+calendarDate);
			...
			setTimeout(function(){
				datenow = $("#datepickerinline").datepicker( 'getDate' );
		...
		$('.choose-date').click(function(){
			...
			//$.scrollTo('#features_1', 1000, { easing: 'easeInOutExpo', offset: 0, 'axis': 'y' });
			 //$('html, body').animate({
			//	scrollTop: $("#datepickerinline").offset().top
			//}, 2000);
			$(".step-2").animate({ scrollTop: $("#datepickerinline").offset().top }, "fast");
			
			$('.goto').addClass('hidden');
			
			//remembering date
			tempdate = $('#datepicker2').val();
			tempdatearr = tempdate.split(' ');
			
			if(isValidDate(tempdatearr[0])){
				dateholder = $('#datepicker2').val();
				//alert('date is valid: '+dateholder);
			}else{
				$('#datepicker2').val(dateholder);
				//alert('date is invalid: '+dateholder);
			}
  ...
  function currDate(){
	...
	$('.stp-count-1').addClass('done');
	$('.stp-count-1 label').html('Tid för avhämtning mailas till dig');	
	$('.stp-count-2').addClass('active');
  }
  ...
  $(function(){
   //acction for invoice
		...
		$('.p-cash').bind('click',function(){
			$('.paymenttype').val('cash');
			$('.radio-btns p a').removeClass('active');
			$(this).addClass('active');
		});
		$('.p-invoice').bind('click',function(){
			$('.paymenttype').val('invoice');
			$('.radio-btns p a').removeClass('active');
			$(this).addClass('active');
		...
		$('.moreinfo').click(function(){
			 $('.seqr-info').removeClass('errormsg').html();  
			 $('.seqr-info .displaymsg').hide()
			$('.fade3, .seqr-info').fadeIn();
			gotoModal();
		});
		
		
		$('.p-seqr').bind('click',function(){
			$('.paymenttype').val('seqr');
			$('.radio-btns p a').removeClass('active');
			$(this).addClass('active');
		});
		...
		$( '.invoice-box' ).delegate( "#save-invoice", "click", function(e) {
			...
			if(erf!=0){
				...
				$('.stp-2').hide();
				$('.stp-3').fadeIn();
				$('.stp-count-2').removeClass('active').addClass('done');
				$('.stp-count-3').addClass('active');
				if($('.paymenttype').val()=='cash'){
					stp2label = "Kort/Kontant vid avhämtning";
				}else{
					stp2label = "Faktura (endast företag)";
				}
				$('.stp-count-2 label').html(stp2label);
			}	
		...
		$('.closebox.iv-box').click(function(e){
			...
			$('.paymenttype').val('cash');
			$('.p-invoice').removeClass('active');
			$('.p-cash').addClass('active');
		});
		...
		$('.stp-1-1 .next').bind('click',function(){
			$('.stp-1-1').hide();
			$('.stp-2').fadeIn();
			$('.stp-count-1').removeClass('active').addClass('done');
			$('.stp-count-2').addClass('active');
			datetime = $('#datepicker2').val();
			
			$('.stp-count-1 label').html(datetime);
			$('.goto').removeClass('hidden');
		});
		
		$('.stp-1-1 .prev').bind('click',function(){
			$('.stp-1-1').hide();
			$('.stp-1').fadeIn();
			
		});
		
		$('.stp-2 .prev').bind('click',function(){
			$('.stp-2').hide();
			if(noSnarast == 1){
				$('.stp-1-1').fadeIn();
			}else{
			...
			$('.stp-count-1').removeClass('done').addClass('active');
			$('.stp-count-2').removeClass('active');
			$('.stp-count-1 label').html('Välj tidpunkt');
			$('.stp-count-2 label').html('Betalning');	
		});
		
		$('.stp-2 .next').bind('click',function(){
			$('.stp-2').hide();
			$('.stp-3').fadeIn();
			$('.stp-count-2').removeClass('active').addClass('done');
			$('.stp-count-3').addClass('active');
			pment = $('.paymenttype').val();
			if(pment=='cash'){
				stp2label = "Kort/Kontant vid avhämtning";
			}else if(pment=='invoice'){
				stp2label = "Faktura (endast företag)";
			}else if(pment='seqr'){
				stp2label = "Betala online med SEQR";
			}
			$('.stp-count-2 label').html(stp2label);	
			
		});
		
		$('.stp-3 .prev').bind('click',function(){
			$('.stp-3').hide();
			$('.stp-2').fadeIn();
			$('.stp-count-2').removeClass('done').addClass('active');
			$('.stp-count-2 label').html('Betalning');
			$('.stp-count-3').removeClass('active');	
		});
		...
		$('*').bind('click, mousemove',function() {
            var containerH = $('.steps-container').height();
			var windowH = $(window).height();
			var top = (windowH - containerH)/2;
			$('.steps-container').css('top',top+'px');
  ...
  function custometimeslider(isClick){
	...
	if(isValidDate(calendarDate)){
		...
		$.ajax({
			...
			success: function(value){
				...
				$('.steps-process .stp-nav .btn.next').fadeIn('fast');
				...
				//console.log('min-max '+dbminHr+':'+dbminMn )
					...
					$('.home .ui-datepicker-inline .ui-timepicker-div').prepend('<div style="padding:10px 0 0; font-size:12px;" class="av-time">Avhämtning erbjuds mellan:<br />'+val[0]+' - '+val[1]+'</div>');
	...
	if(isClick == 1){
		setTimeout(function(){
			...
			$('#datepicker2').val(datenow+' '+$(".starttime1").val());	
			//alert('aaa');
...
<style>
	...
	.home .ui-timepicker-div,
	.home .ui-datepicker-buttonpane {
		float: right !important;
		width: 47% !important;
		text-align: center;
	}
	.home .ui-datepicker-inline{
		width:100%;	
	}
	.home .ui-datepicker-inline .ui-timepicker-div{
		width:47% !important;	
	}
	.home .ui-datepicker-inline .ui-datepicker-buttonpane{
		display:none;	
	}
	...
	.radio-btns p a.btn.active{
		background: #c4db5b !important;	
	}
	...
	.steps-process .stp-nav .btn.prev{
		margin-left:0;	
	}
	.steps-process .stp-nav .btn.next{
		margin-right:0;	
	}
	...
	@media only screen and (max-width: 568px){
		...
		div#datepickerinline{
			position: relative;
			z-index: 999;	
		}	
		.steps-process .stp .st-col input[type=text],
		.steps-process .stp .st-col input[type=password] {
			width: 100% !important;
			padding: 10px;
			margin-bottom: 10px;
		}
		...
		.steps-process .ui-widget .ui-widget {
			padding: 10px 32px;
		}
	}
</style>
<div class="steps-container">
	...
	if($noSettings>0){ ?>
        <div class="steps-content">
            ...
            <?php /*?><ul style="margin: 7px auto 0;" class="radiousers">
                <li>
                ...
                <li>
                    ...
                    <div class="st-col">
                        ...
                        <input type="checkbox" id="remember"  class="tos remember"><label for="remember" class="" style="font-size: 12px;">Kom ihåg mig</label><br />
                    </div>
                </li>
                <li class="exists">
                    	<b class="label">4.</b>
                        <div class="st-col">
                        	<input type="checkbox" id="agree11" class="tos" checked="checked"><label for="agree11" class="agreelabel1">Jag har läst och godkänner användarvillkoren</label> 
                        </div>
                </li>
            ...
            <div class="steps-counter">
            	<div class="stp-count-1 active"><span>1</span><label>Välj tidpunkt</label></div>
                <div class="stp-count-2"><span>2</span><label>Betalning</label></div>
                <div class="stp-count-3"><span>3</span><label>Mina uppgifter</label></div>
            </div>
            <div class="steps-process">
            	<div class="stp stp-1">
                	<h2>När vill du ha din beställning klar?</h2>
                    <div class="box-cont">
                        <input type="button" value="SNARAST" data-rel="step-2"  class="current-date btn">
                        <input type="button" value="ANNAN TID" data-rel="step-2"  class="choose-date btn">
                    </div>
                </div>
                <div class="stp stp-1-1">
                	<input type="hidden" class="t-input" id="datepicker2" value="Välj tid och datum" />
                    <div id="datepickerinline"></div>
                    <div class="stp-nav">
                    	<a href="#back" class="prev btn" style="display:none;"></a>
                        <a href="#next" class="next btn">Nästa</a>
                    </div>
                </div>
                <div class="stp stp-2">
                	<h2>Välj betalsätt</h2>
                    <input type="hidden" class="paymenttype"  value="cash"/>
                    <div class="radio-btns">
                        <p><a href="#" class="btn p-cash active">Kort/Kontant vid avhämtning</a></p>
                        <p><a href="#" class="btn p-invoice">Faktura (endast företag)</a></p>
                        <p style="display:none;"><a href="#" class="btn p-seqr">Betala online med SEQR</a>
                        	<span class="moreinfo"><a href="#">(mer information)</a></span>
                        </p>
                    </div>
                    <div class="stp-nav">
                    	<a href="#back" class="prev btn"></a>
                        <a href="#next" class="next btn">Nästa</a>
                    </div>
                </div>
                <div class="stp stp-3">
                	<div class="st-col">
                        <h2>Uppge dina användaruppgifter</h2>
                        <a href="javascript:void(0)" style="font-size: 13px; float:none;" class="ifnew">Skapa ett konto</a><br />
                    
                        <input type="text" class="takeemail t-input" placeholder="*E-post"><br />
                        <input type="password" class="takepass t-input" placeholder="*Lösenord"><br />
                        <a href="javascript:void(0)" class="forgot"  style="font-size: 13px;">Klicka här om du har glömt ditt lösenord</a><br /><br />
                    ...
                    <div class="stp-nav">
                    	<a href="#back" class="prev btn"></a>
                        <a href="#checkout" class="exists boka btn">BESTÄLL</a>
        ...
        <?php /*?><div class="steps-button-2" style="padding:0;">
...
<?php /*?><div class="invoice-box modalbox" style="display:none;">
	<h2>Faktureringsinformation<a href="javascript:void(0)" class="closebox iv-box"></a></h2>
    <div class="box-content">
...
<div class="seqr-box modalbox" style="display:none;">
	<h2>SEQR<a href="javascript:void(0)" class="closebox seqr-box"></a></h2>
    <div class="box-content">
     	
    </div>
</div>

<div class="seqr-info modalbox" style="display:none;">
	<h2>SEQR<a href="javascript:void(0)" class="closebox seqr-info"></a></h2>
    <div class="box-content">
        <p><iframe id="seqr-video" class="seqr-video" width="360px" height="203px" src="https://www.youtube.com/embed/GST4ofKoEM4?rel=0&amp;showinfo=0&amp;theme=light&amp;modestbranding=1&amp;vq=hd720&amp;controls=0&enablejsapi=1" frameborder="0"></iframe></p><br />
        <p><center>Ladda ner SEQR till din smartphone</center></p><br />
        <ul class="seqr-download">         <li>             <a href="https://itunes.apple.com/app/seqr/id494224742?l=sv&amp;mt=8" target="_blank">                 <img src="https://cdn.seqr.com/webshop-plugin/images/iphone.svg"><br>iPhone             </a>         </li>         <li>             <a href="https://play.google.com/store/apps/details?id=com.seamless.seqr" target="_blank">                 <img src="https://cdn.seqr.com/webshop-plugin/images/android.svg"><br>Android             </a>         </li>         <li>             <a href="http://www.windowsphone.com/sv-se/store/app/seqr/7c25ac1c-dfb7-4cd5-b040-fdeef6ab09a8" target="_blank">                 <img src="https://cdn.seqr.com/webshop-plugin/images/windows.svg"><br>Windows             </a>         </li>     </ul>
    </div>
</div>
...
$(function(){	
		...
		$('.seqr-box h2 a').click(function(){
			$('.seqr-box, .fade3').fadeOut();
		});
		
		$('.seqr-info h2 a').click(function(){
			$('.seqr-info, .fade3').fadeOut();
            document.getElementById('seqr-video').contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
		});
